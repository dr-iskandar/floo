...
namespace UnityEngine.UI.Extensions
{
    ...
    {
		//ww COSTUMIZED :: 
		public GameObject ParentScrollObj;
		private ScrollRect ParentScrollRect;
		private HorizontalScrollSnap scriptParent;
        ...
        void Start()
        {
			//ww COSTUMIZED :: Get the current scroll rect so we can disable it if the other one is scrolling
			if (ParentScrollRect == null)
			{
				ParentScrollRect = ParentScrollObj.GetComponent<ScrollRect>();
			}

			if (ParentScrollObj.GetComponent<HorizontalScrollSnap>() != null)
			{
				scriptParent = ParentScrollObj.GetComponent<HorizontalScrollSnap>();
			}
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (scrollOtherHorizontally)
            {
                if (horizontal > vertical)
                {
					...
					{
						scriptParent.OnBeginDrag(eventData);
					}
                }
            }
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            if (scrollOther)
            {
				...
				if (scriptParent != null)
				{
					scriptParent.OnEndDrag(eventData);
				}

				_myScrollRect.verticalNormalizedPosition = 1;
				_myScrollRect.horizontalNormalizedPosition = 1;
            }
        }
